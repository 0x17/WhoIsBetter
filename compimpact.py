...
def comp_measurefunc(res_dict, m_a, m_b):
    ...
    nwin_a, nwin_b = domination_count_pair(res_dict, m_a, m_b, instances, consider_time=False)
    return competitiveness_measures(nwin_a, nwin_b, len(instances))


def perf_measurefunc(res_dict, m_a, m_b, obj = 'profit'):
    if obj == 'profit':
        selfunc = max
        objfunc = profit
    else:
    ...
    instances = all_instances_for_pair(res_dict, m_a, m_b)
    single_algo_profits = [sum(objfunc(res_dict, mn, inst) for inst in instances) for mn in [m_a, m_b]]
    oracle_profit = sum(selfunc(objfunc(res_dict, mn, inst) for mn in [m_a, m_b]) for inst in instances)
    s_a = dominated_instances(res_dict, m_a, m_b, instances, consider_time=obj != profit)
    s_b = dominated_instances(res_dict, m_b, m_a, instances, consider_time=obj != profit)
    return {
